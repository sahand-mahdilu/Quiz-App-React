...
export default function QuizApp() {
  const [Question, setQusertion] = useState([
    ...
    {
      questionText:
        "which of the following is a popular java-script library for user inter face?",
      id: 3,
              ...
              {Question[currentQuestion].answers.map((answer) => {
                return (
                  <button
                    key={answer.id}
                    onClick={() => {
                      clickHandler(answer.isCorrect);
                    }}
                    className="hover:bg-blue-700 p-1 border rounded-md w-36"
                  >
                    {answer.answerText}
                  </button>
                );
